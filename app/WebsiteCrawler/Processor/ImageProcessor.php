...
class ImageProcessor implements ProcessorInterface
{
    ...
    {
        foreach ($this->normalizeUris($result->getUri(), $result->get($this->getType())) as $imageUri) {
            try {
                $response = $context->getHttpClient()->sendRequest(
                    $context->getMessageFactory()->createRequest('GET', $imageUri)
                );
            ...
            $this->filesystem->put(
                sprintf('%s/%s', $result->getUri()->getHost(), $this->normalizeFileName($imageUri)),
                $response->getBody()
            );
        }
    }
